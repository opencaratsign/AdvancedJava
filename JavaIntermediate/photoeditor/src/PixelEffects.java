...
public class PixelEffects {
	...
	public static int[][] resize(int[][] source, int newWidth, int newHeight) {
		...
		for (int x = 0 ; x < newWidth; x++) {
			for (int y = 0; y < newHeight; y++) {
				int sourceX = (int) (x * (((double)source.length) / ((double)newWidth)));
				int sourceY = (int) (y * (((double)source[0].length) / ((double)newHeight)));
	...
	public static int[][] merge(int[][] sourceA, int[][] sourceB) {
		...
		for (int x = 0 ; x < sourceWidth; x++) {
			for (int y = 0; y < sourceHeight; y++) {
				...
				int pixel = RGBUtilities.toRGB(
						(redValue + redValue2) / 2, 
						(greenValue + greenValue2) / 2, 
						(blueValue + blueValue2) / 2
					);
				output[x][y] = pixel;
			}
		}
	...
	public static int[][] chromaKey(int[][] foreImage, int[][] backImage) {
		int sourceWidth = foreImage.length;
		int sourceHeight = foreImage[0].length;
		int[][] output = new int[sourceWidth][sourceHeight];
		backImage = resize(backImage, foreImage);
		for (int x = 0 ; x < sourceWidth; x++) {
			for (int y = 0; y < sourceHeight; y++) {
				int greenValue = RGBUtilities.toGreen(foreImage[x][y]);
				int redValue = RGBUtilities.toRed(foreImage[x][y]);
				int blueValue = RGBUtilities.toBlue(foreImage[x][y]);
				if (greenValue > 220 && redValue < 40 && blueValue < 40) {
